...
class RegexCompiledTailorer(ResumeTailorer):
    def __init__(self):
        pass

    def _calculate_relevance(
        self, experience: Dict[str, Any], keywords: List[str], compiled_pattern=None
    ) -> float:
        ...
        if compiled_pattern:
            matches = compiled_pattern.findall(text_to_check)
...
def run_benchmark():
    tailorer = RegexCompiledTailorer()
    ...
    for _ in range(100):
        ...
        for exp in resume_data["experience"]:
            tailorer._calculate_relevance(exp, keywords, compiled_pattern=pattern)
