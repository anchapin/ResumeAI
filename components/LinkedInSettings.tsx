...
export const LinkedInSettings: React.FC = () => {
  ...
  const handleDisconnect = async () => {
    if (!confirm('Are you sure you want to disconnect your LinkedIn account? This will revoke access to your profile data.')) {
      return;
    }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg">
                    ...
                    <p className="font-medium text-slate-900">{profile.headline || 'Not specified'}</p>
                  </div>
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {profile.experience.slice(0, 3).map((exp, idx) => (
                        <div key={idx} className="flex items-start gap-2">
                          <span className="material-symbols-outlined text-slate-400 text-[16px] mt-0.5">work</span>
                          <div>
                ...
                {profile.skills && profile.skills.length > 0 && (
                  <div className="bg-slate-50 p-4 rounded-lg">
                    <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Skills ({profile.skills.length})</p>
                    <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                      {profile.skills.slice(0, 10).map((skill, idx) => (
                        <span key={idx} className="px-2.5 py-1 bg-white border border-slate-200 rounded-md text-xs font-medium text-slate-600">
                          {skill}
                        </span>
                      ))}
