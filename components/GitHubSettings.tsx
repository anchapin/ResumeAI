...
async function processGitHubCallback(code: string, state: string): Promise<void> {
  ...
  const response = await fetch(
    `${API_URL}/api/v1/github/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
...
const GitHubSettings: React.FC = () => {
  ...
  const handleDisconnectGitHub = async () => {
    if (
      !confirm(
        'Are you sure you want to disconnect your GitHub account? This will revoke access to your GitHub data.',
      )
    ) {
      return;
    }
    ...
    <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
        ...
        <p className="text-sm text-slate-500">
          Connect your GitHub account to import projects and contributions
        </p>
      </div>
      <div className="p-6">
        {/* Loading State */}
        {isLoading && !isConnecting && (
          <div
            ...
            role="status"
            aria-label="Loading GitHub settings"
          >
            <span
          ...
          <div className="space-y-4">
            <div className="text-center py-6">
              ...
              <p className="text-slate-500 text-sm mb-4">
                Link your GitHub account to import your projects, repositories, and contributions to
                your resume.
              </p>
              <button
            ...
            <div className="bg-slate-50 rounded-lg p-4">
              <h5 className="font-bold text-slate-900 text-sm mb-3">
                What you can do with GitHub connected:
              </h5>
              <ul className="space-y-2">
                <li className="flex items-start gap-2 text-sm text-slate-600">
                  <span
                    className="material-symbols-outlined text-primary-600 text-[18px] mt-0.5"
                    aria-hidden="true"
                  >
                    check
                  </span>
            ...
            <div className="flex items-start gap-2 text-xs text-slate-500 bg-blue-50 p-3 rounded-lg">
              <span
                className="material-symbols-outlined text-blue-600 text-[16px] mt-0.5"
                aria-hidden="true"
              >
                info
              </span>
              <p>
                We only request read access to your public profile and email. Your data is securely
