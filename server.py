...
def json_resume_to_cli_yaml(json_resume: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    print(
        f"DEBUG: Processing incoming resume data for: {json_resume.get('basics', {}).get('name')}"
    )
    sys.stdout.flush()
    ...
    cli_data = {
        ...
        "projects": {},
        "skills": {},
        "variants": {"base": {"summary_key": "base"}},
    }
            ...
            featured_projects.append(
                {
                    ...
                    "highlighted_technologies": proj.get("highlights")
                    or proj.get("tags")
...
async def v1_render_markdown(request: V1RenderPdfRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Markdown generation failed: {str(e)}"
        )
...
async def import_linkedin_file(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        raise Exception(
            "No valid LinkedIn data found in files (expected .json, .zip, or .csv)"
        )
    ...
    for loc_key in ["location", "locationName", "geoLocation", "address", "city"]:
        if loc_key in profile:
            ...
            if isinstance(loc, dict):
                loc_name = (
                    loc.get("name") or loc.get("city") or loc.get("preferredName") or ""
                )
            else:
                loc_name = str(loc)
            if loc_name:
        ...
        resume_data["work"].append(
            {
                ...
                "endDate": (
                    f"{pos.get('finishedOn', {}).get('month', '01')}/{pos.get('finishedOn', {}).get('year', '')}"
                    if pos.get("finishedOn")
                    else "Present"
                ),
...
def _process_linkedin_csvs(csv_files):
    ...
    for skill in linkedin_data.get("skills", []):
        name = skill.get("Name")
        if name:
            resume_data["skills"].append(name)
