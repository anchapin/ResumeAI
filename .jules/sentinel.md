...
## 2026-06-08 - [SQL Injection in migration_helpers.py via text()]

**Vulnerability:** The query execution around line 314 in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically, creating a SQL injection vulnerability if `table_name` or `id_column` are user-controlled.
**Learning:** Using f-strings inside `sqlalchemy.text()` does not sanitize inputs and is vulnerable to SQL injection.
**Prevention:** Use SQLAlchemy Core constructs like `table()` and `column()` combined with `select()` for dynamic table and column identifiers.
