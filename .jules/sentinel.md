...
## 2026-06-15 - [SQL Injection in Batch Migrations]

**Vulnerability:** The `migrate_table_in_batches` function in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}...")`), creating a SQL injection vulnerability if those identifiers are user-controlled.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via Python string formatting. Using standard string concatenation in database execution is inherently dangerous. This was previously identified in `schema_manager.py` but the pattern existed elsewhere.
**Prevention:** To safely use dynamic identifiers, use SQLAlchemy core objects like `table()` or `column()` combined with `select()`.
