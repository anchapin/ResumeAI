...
## 2026-05-25 - [SQL Injection via String Formatting in SQLAlchemy text()]
**Vulnerability:** The `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically, creating a SQL injection vulnerability.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via Python string formatting (like f-strings). Using standard string concatenation in database execution is inherently dangerous.
