...
## 2026-04-11 - [SQL Injection via String Formatting in SQLAlchemy text() inside migration_helpers.py]

**Vulnerability:** The `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}")`), creating a direct vector for SQL injection if `table_name` or `id_column` is user-controlled.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via Python string formatting (like f-strings or `.format()`). Using standard string concatenation in database execution is inherently dangerous, even in seemingly internal migration utilities.
**Prevention:** To safely use dynamic identifiers (like table or column names), use SQLAlchemy core objects like `table()` or `column()` combined with `select()` and `order_by()`. Never use string formatting for table or column names.
