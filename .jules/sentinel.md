...
## 2024-05-18 - [SQL Injection via String Interpolation in SQLAlchemy `text()`]

**Vulnerability:** The `_get_table_row_count` function in `resume-api/lib/db/schema_manager.py` used an f-string to interpolate user-provided table names directly into `sqlalchemy.text()`. This creates a severe SQL Injection vulnerability, as malicious input could execute arbitrary SQL commands.
**Learning:** String interpolation or concatenation should never be used to construct SQL queries, even when using SQLAlchemy's `text()` function.
**Prevention:** To prevent SQL injection when using SQLAlchemy, avoid passing Python f-strings or string interpolation directly into `text()`. For dynamic table or column identifiers, use SQLAlchemy core constructs like `select()`, `table()`, and `func`.
