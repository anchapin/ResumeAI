...
## 2024-12-16 - Subprocess Argument Injection and Resource Exhaustion
**Vulnerability:** The `is_ecryptfs_path` function executed `subprocess.run(["df", "-Th", path])` without separating options from positional arguments or specifying a timeout limit.
**Learning:** If `path` is controlled by a user and starts with a hyphen (e.g. `-file`), `df` will treat it as a command-line option rather than a file path, leading to argument injection. Furthermore, the lack of a timeout means that if the `df` process hangs (e.g., due to a stuck network mount), the API thread will block indefinitely, leading to resource exhaustion (DoS).
**Prevention:** Always use `--` to signify the end of options when passing user-controlled variables to system commands via `subprocess.run` (e.g., `["df", "-Th", "--", path]`). Additionally, always provide a strict `timeout` parameter to prevent process hangs from monopolizing system resources.
