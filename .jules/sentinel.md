...
## 2024-05-24 - Avoid Fast Hashing Algorithms for Password Storage
**Vulnerability:** Found `hashlib.sha256` being used for hashing and verifying passwords on shared resumes (`share_password_hash`).
**Learning:** Raw fast hashing algorithms like `hashlib.sha256` are vulnerable to brute-force and dictionary attacks because they execute too quickly. Password hashes must be deliberately slow to mitigate these attacks.
**Prevention:** Use standard, well-tested password hashing utilities (`bcrypt` via `config.security.hash_password` and `verify_password`) rather than implementing custom or fast cryptographic hashing for passwords.
