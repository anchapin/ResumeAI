...
## 2024-04-26 - [Prevent SQL Injection using SQLAlchemy]
**Vulnerability:** Used `text(f"...")` in SQLAlchemy raw SQL query with dynamic variables (e.g. `id_column` and `table_name`) leading to SQL Injection vulnerabilities.
**Learning:** SQL Injection vulnerabilities can happen even when using modern ORMs if developers construct raw SQL text utilizing string formatting (`f"..."`) incorporating parameters directly into the query instead of parameterized binding or the ORM's core APIs.
**Prevention:** Avoid `text()` with string interpolation. Always use parameter bindings, or preferably use SQLAlchemy core API constructs like `select()`, `table()`, and `column()` for dynamic table or column name handling.
