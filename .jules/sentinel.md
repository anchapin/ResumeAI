...
## 2024-05-18 - CI Failures specific to Vitest upgrades
**Vulnerability:** Upgrading `vitest` and `@vitest/coverage-v8`, `@vitest/coverage-istanbul` to `>= 4.1.0` causes import resolution failures on `vitest.config.ts`.
**Learning:** `Could not resolve 'vitest/config' in vitest.config.ts` occurs in GitHub Actions and local tests.
**Prevention:** Hardcode vitest packages to `^4.0.18`.
