...
## 2026-04-10 - [SQL Injection via String Formatting in SQLAlchemy text() in BatchMigrationManager]

**Vulnerability:** The `BatchMigrationManager.migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}")`), creating a vector for SQL injection if `table_name` or `id_column` are improperly sanitized.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via Python string formatting. Using standard string concatenation in database execution is inherently dangerous.
**Prevention:** To safely use dynamic identifiers (like table or column names), use SQLAlchemy core objects like `table()` or `column()` combined with `select()`.
