...
## 2026-05-11 - [SQL Injection via String Formatting in SQLAlchemy text()]
**Vulnerability:** Similar to a previous finding, the `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}...")`), creating a direct vector for SQL injection if `table_name` or `id_column` are user-controlled or dynamically generated.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via Python string formatting (like f-strings or `.format()`). Even in internal database helper scripts, using standard string concatenation in database execution is inherently dangerous.
**Prevention:** Always construct queries dealing with dynamic identifiers using SQLAlchemy's Core API objects, such as `select()`, `table()`, and `column()`, and explicitly use `bindparam` with bounded execution arguments.
