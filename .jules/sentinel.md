...
## 2026-05-28 - [XSS Bypass via Unquoted Event Handlers]
**Vulnerability:** XSS filter in sanitize_html failed to remove unquoted on* event handlers (e.g. onerror=alert(1)).
**Learning:** Regular expressions for attribute removal must account for both quoted and unquoted values to prevent bypasses.
**Prevention:** Always use robust HTML parsing libraries for sanitization, or if using regex, ensure it captures `[^\s>]+` for unquoted attributes.
