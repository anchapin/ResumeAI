...
## 2026-04-03 - [SQL Injection via String Formatting in BatchMigrationManager]

**Vulnerability:** The `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used an f-string within `sqlalchemy.text()` to build a paginated SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}")`). This created a SQL injection vulnerability if `table_name` or `id_column` were user-controlled.
**Learning:** Even internal utility functions like migration helpers can introduce critical SQL injection risks if they dynamically format SQL queries using `text()`, circumventing SQLAlchemy's parameter binding.
**Prevention:** Always use SQLAlchemy's core query builder functions, such as `table(table_name, column(id_column))` combined with `select()`, which safely escapes and quotes dynamic table and column names automatically.
