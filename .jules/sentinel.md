...
## 2024-05-18 - [SQL Injection via Dynamic SQLAlchemy text() Table Names]
**Vulnerability:** The `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used a Python f-string within `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}")`), creating a direct vector for SQL injection if `table_name` or `id_column` are ever controlled by user input or unsafe config.
**Learning:** Using `sqlalchemy.text()` with string interpolation for dynamic identifiers (like table names or column names) bypasses parameterization and creates critical SQL injection vulnerabilities, even if the identifiers aren't directly exposed to users.
**Prevention:** Always use SQLAlchemy's Core API objects, such as `select()`, `table()`, and `column()`, to construct queries with dynamic identifiers safely instead of string concatenation or interpolation.
