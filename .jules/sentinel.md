...
## 2026-06-23 - [SQL Injection via String Formatting in SQLAlchemy text()]
**Vulnerability:** The `migrate_table_in_batches` method in `resume-api/lib/db/migration_helpers.py` used an f-string inside `sqlalchemy.text()` to build a SQL query dynamically (`text(f"SELECT {id_column} FROM {table_name}")`), creating a direct vector for SQL injection.
**Learning:** `sqlalchemy.text()` does NOT sanitize variables passed into it via string formatting. This anti-pattern can lead to critical SQLi if the variables are ever influenced by user input.
**Prevention:** Always use SQLAlchemy core objects like `table()` or `column()` combined with `select()` for dynamic identifiers to ensure proper quoting and parameterization.
