...
## 2026-03-30 - Added aria-pressed state to comment filter toggle buttons
**Learning:** Screen readers cannot determine the active state of visually distinct toggle buttons (like "All" vs "Unresolved" in comment filtering) unless explicitly told using the `aria-pressed` attribute.
**Action:** When implementing custom toggle buttons that filter or change views, ensure they include `aria-pressed={isActive}` so their state is accessible to screen readers, alongside any visual class changes.
