...
## 2023-10-25 - N+1 Query Patterns in Batch Routes
**Learning:** Batch operations (e.g., `batch_export`, `batch_delete`, `batch_update`, `bulk_operations`) often inadvertently contain N+1 query problems by fetching database items inside loops, which severely degrades API performance and connection pool utilization when scaling batch sizes.
**Action:** Always extract database queries outside of loops using the SQLAlchemy `.in_()` operator, map the results to a dictionary for O(1) lookups, and perform business logic iteration over the in-memory dictionary.
