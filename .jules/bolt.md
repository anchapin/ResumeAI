...
## 2026-03-27 - SQLAlchemy N+1 Queries
**Learning:** Performing `db.execute(select(Model).where(Model.attr == item))` inside a loop over an array of items (like tags for a resume) creates an N+1 query pattern that severely degrades performance when updating/creating resources with many associations.
**Action:** Always replace iterative database lookups within loops with a single bulk query using `Model.attribute.in_(list_of_values)`, mapping results to a dictionary for O(1) retrieval during association, and calling `db.flush()` once afterward if new models are added.
