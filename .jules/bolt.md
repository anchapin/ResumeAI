...
## 2026-03-26 - Optimized N+1 query in Resume Listing
**Learning:** To eliminate N+1 query patterns when returning associated entity counts in SQLAlchemy, use scalar subqueries instead of iterating through joined relationships via `len(obj.rel)` in python space. Also, remember to add `noload(Model.relationship)` to the query options if the relationship defaults to eager loading to prevent fetching all nested objects into memory unnecessarily. In a paginated route returning resumes and their version count, a subquery calculating `func.count(ResumeVersion.id)` mapped to the main query along with `noload(Resume.versions)` completely resolves the query overhead.
**Action:** Always prefer `scalar_subquery()` integrated into the main `select()` combined with `result.all()` rather than accessing deeply loaded relationships inside python list comprehensions, especially for simple aggregations like counts, and specifically ensure you instruct SQLAlchemy to ignore loading the relationship by using `.options(noload())`.
