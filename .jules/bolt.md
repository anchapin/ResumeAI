...
## 2026-03-24 - Python Multiple Substring Matching Optimization
**Learning:** Using `any(word in string for word in list_of_words)` forces iteration at the Python level. Even when searching for simple static strings without regex tokens, compiling them into a single regex `pat = re.compile(r"word1|word2|word3")` and using `pat.search(string)` is significantly faster (around ~20-30% faster in our benchmarks). This leverages the highly optimized C-level regular expression engine rather than executing Python bytecode iteratively.
**Action:** When writing scoring or classification logic that checks a single text against multiple static keywords or substrings inside a loop, pre-compile all possible target words into a module-level `re.compile(r"word1|word2")` regular expression to improve performance.
