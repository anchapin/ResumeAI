...
## 2026-03-11 - Hoisting static lists and Batching string searches
**Learning:** In text processing applications where a static list of terms (like 80+ technology skills) is checked against many chunks of text (e.g., individual bullet points in a resume), allocating the list within the function scope creates an `O(N*M)` memory and execution overhead (where N is the number of text chunks and M is the number of terms). By moving the list to a module-level global `tuple` and batching all text chunks into a single string (`" ".join(texts)`) before searching, performance improves significantly (~25-30% faster in benchmarks) while producing the identical deduplicated output.
**Action:** Always hoist static, immutable reference lists or sets out of frequently called text-processing functions to the global scope as `tuple` or `frozenset`. If the final output is a unique set of matched terms, batch the search domain into a single concatenated string rather than repeatedly searching over granular chunks.
