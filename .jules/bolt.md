...
## 2024-06-13 - Avoid Repeated Lowercasing in Loops
**Learning:** In text scoring loops, iterating over keywords that could be pre-lowercased outside the loop results in redundant `.lower()` operations for every keyword against every item. When the caller passes lowercased keywords, lowercasing them again inside the scoring function wastes time.
**Action:** For text processing and scoring performance, pre-calculate case manipulations (like `.lower()`) on static lists or search sets at the highest logical scope possible, passing the pre-computed structures down to avoid redundant string allocations within tight loops.
