...
## 2026-04-21 - Intl.NumberFormat Instantiation Optimization
**Learning:** Repeatedly instantiating `Intl.NumberFormat` inside React render cycles or formatting loops (e.g., when rendering multiple job offers in a comparison table) is a significant performance bottleneck due to the high cost of initializing the formatter object.
**Action:** Always cache and reuse `Intl.NumberFormat` instances using a module-level `Map` or similar cache, where the key incorporates the currency code and any varying options (e.g., minimum/maximum fraction digits), rather than calling `new Intl.NumberFormat` inline.
