...
## 2024-05-30 - Fix N+1 Query in Batch Operations
**Learning:** In SQLAlchemy, iteratively selecting individual rows within a loop for bulk operations (like batch updates or batch deletes) creates an N+1 query bottleneck. While `selectinload` helps with eager loading relationships for each individual query, it doesn't prevent the loop itself from executing a separate query for each requested item.
**Action:** For batch processing functions, always hoist the database queries outside the iteration loop by pre-fetching the necessary records using `.in_(list_of_ids)`. Store the results in an in-memory dictionary for O(1) lookups inside the loop. Additionally, when using `.in_()` outside loops and processing individual items within the loop, avoid calling `await db.commit()` or `await db.rollback()` inside the loop, as doing so expires the prefetched objects and leads to `MissingGreenlet` errors. Instead, wrap inner-loop operations in `async with db.begin_nested():` (savepoints) to handle partial failures gracefully, and perform a single `await db.commit()` at the end of the function.
