...
class LoadTestResults:
    ...
    def finalize(self, total_duration: float):
        ...
        self.queries_per_second = self.total_queries / total_duration if total_duration > 0 else 0
        ...
        if successful_times:
            ...
            if len(successful_times) > 1:
                ...
                self.p95_query_time_ms = sorted_times[min(p95_idx, len(sorted_times)-1)]
                self.p99_query_time_ms = sorted_times[min(p99_idx, len(sorted_times)-1)]
    ...
    def print_summary(self):
        ...
        print(f"Successful:           {self.successful_queries} ({self.success_rate:.1f}%)")
...
@click.option('--num-queries', default=1000, type=int,
              help='Number of queries for each test')
@click.option('--concurrent', default=10, type=int,
