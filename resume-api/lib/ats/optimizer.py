...
class ATSOptimizer:
    ...
    def get_ats_aware_score(
        self, job_description: str, resume_data: dict, ats_score: float, base_match_score: float
    ) -> ATSAwareScore:
        """
    ...
    def _generate_recommendations(
        self, ats_score: float, base_match_score: float, job_description: str, resume_data: dict
    ) -> list[str]:
    ...
    def get_optimization_priority(self, ats_score: float, match_score: float) -> str:
        """
    ...
    def estimate_improvement(
        self, current_ats_score: float, target_ats_score: float, current_match_score: float
    ) -> dict:
        """
        ...
        percentage_improvement = (
            (score_improvement / current_adjusted * 100) if current_adjusted > 0 else 0
        )
    ...
    def _get_optimization_strategy(self, current_ats_score: float, target_ats_score: float) -> str:
