...
class LogScrubber:
    ...
    DEFAULT_RULES: List[ScrubRule] = [
        ...
        ScrubRule(
            name="aws_secret",
            pattern=re.compile(r"(?i)aws[_-]?secret[_-]?access[_-]?key[\"']?\s*[:=]\s*[\"']?([a-zA-Z0-9/+=]{40})[\"']?", re.IGNORECASE),
        ...
        ScrubRule(
            name="secret",
            pattern=re.compile(r"(?i)(secret|client[_-]?secret)[\"']?\s*[:=]\s*[\"']?([^\s\"'&]+)[\"']?", re.IGNORECASE),
            replacement="[SECRET]"
        ),
        ScrubRule(
            name="private_key",
    ...
    def scrub_dict(self, data: Dict[str, Any], keys_to_scrub: Optional[List[str]] = None) -> Dict[str, Any]:
        """
