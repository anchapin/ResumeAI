...
class ApplicationStorage:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        if storage_path is None:
            ...
            # If in a container or restricted environment, fallback to a local temp dir
            try:
                user_data_dir = Path.home() / ".resumeai" / "data"
                user_data_dir.mkdir(parents=True, exist_ok=True)
            except PermissionError:
                # Fallback to local app directory for containerized environments
        ...
        for i, app_data in enumerate(data):
            if app_data.get("id") == app_id:
                ...
                if "applied_date" in updates and updates["applied_date"]:
                    application.applied_date = datetime.fromisoformat(
    ...
    def add_reminder(
        self, app_id: str, message: str, remind_at: datetime
