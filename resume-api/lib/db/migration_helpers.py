    ...
    async def migrate_table_in_batches(
        ...
        while True:
            ...
            async with self.primary_engine.begin() as conn:
                # Use Core constructs to avoid SQL injection on table and column names
                target_table = table(table_name, column(id_column))
                stmt = select(target_table.c[id_column]).order_by(target_table.c[id_column]).limit(self.batch_size).offset(offset)
