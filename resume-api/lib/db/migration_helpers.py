    ...
    async def migrate_table_in_batches(
        ...
        while True:
            ...
            async with self.primary_engine.begin() as conn:
                # Use SQLAlchemy Core constructs instead of f-strings to prevent SQL injection
                stmt = select(column(id_column)).select_from(table(table_name)).order_by(column(id_column)).limit(self.batch_size).offset(offset)
