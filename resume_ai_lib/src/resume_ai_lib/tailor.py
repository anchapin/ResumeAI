...
class ResumeTailorer:
    ...
    def tailor_resume(
        ...
        if self.client:
            try:
                tailored_data = self._ai_tailor(tailored_data, job_description, keywords)
            except Exception as e:
                logger.warning(f"AI tailoring failed, using keyword-based tailoring: {e}")

        logger.info("Resume tailoring completed")
        return tailored_data

    def _calculate_relevance(self, experience: Dict[str, Any], keywords: List[str]) -> float:
...
Return ONLY valid JSON, nothing else."""
    ...
    def _regex_extract_keywords(self, job_description: str) -> List[str]:
        ...
        tech_keywords = [
            ...
            "ruby", "php", "swift", "kotlin", "scala", "haskell", "sql",
            "react", "vue", "angular", "node.js", "django", "flask", "fastapi",
            "spring", "rails", "laravel", "next.js", "nuxt", "express",
        ...
        for word in capitalized:
            ...
            if word_lower not in found_keywords and len(word) > 3:
                # Add common ones
                if word_lower not in ["and", "the", "for", "with", "from", "this", "that"]:
