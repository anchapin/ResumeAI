...
export async function exportCoverLetterToPDF(
  ...
  try {
    const response = await fetch(`${API_URL}/api/v1/render/pdf`, {
      ...
      body: JSON.stringify({
        resume_data: { basics: { name: '', summary: content } },
 ...
 * @param options - Formatting options
 */
export async function exportCoverLetterToWord(
  content: string,
  options: FormatOptions = DEFAULT_FORMAT_OPTIONS,
): Promise<void> {
  const html = generateCoverLetterHTML(content, options);
  const blob = new Blob([html], {
    type: 'application/vnd.ms-word',
  });
  const url = window.URL.createObjectURL(blob);
 ...
 * @param content - Cover letter content
 */
export async function exportCoverLetterToTXT(content: string): Promise<void> {
...
/**
 * Export cover letter to Markdown
 * @param content - Cover letter content
 */
export async function exportCoverLetterToMarkdown(content: string): Promise<void> {
  const blob = new Blob([content], { type: 'text/markdown' });
  const url = window.URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `cover-letter-${Date.now()}.md`;
  document.body.appendChild(a);
  a.click();
  window.URL.revokeObjectURL(url);
  document.body.removeChild(a);
...
/**
 * Generate HTML from cover letter content
 */
function generateCoverLetterHTML(content: string, options: FormatOptions): string {
  const styles = getHTMLStyles(options);
  return `<!DOCTYPE html>
<html>
<head>
