...
describe('API Client - Retry Logic Integration', () => {
  ...
  describe('createResume with retry', () => {
    it('retries on 503 Service Unavailable', async () => {
      fetchSpy
        ...
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Service Unavailable' }), { status: 503 }))
        .mockResolvedValueOnce(new Response(JSON.stringify({ id: 1, title: 'Test Resume' }), { status: 200 }));
    ...
    it('retries on network error', async () => {
      fetchSpy
        ...
        .mockResolvedValueOnce(new Response(JSON.stringify({ id: 2, title: 'Another Resume' }), { status: 200 }));
  ...
  describe('getResume with retry', () => {
    it('succeeds after retry on 502 Bad Gateway', async () => {
      fetchSpy
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Bad Gateway' }), { status: 502 }))
        .mockResolvedValueOnce(new Response(JSON.stringify({ id: 1, title: 'Retrieved Resume' }), { status: 200 }));
    ...
    it('retries on rate limiting (429)', async () => {
      fetchSpy
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Too Many Requests' }), { status: 429 }))
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Too Many Requests' }), { status: 429 }))
        .mockResolvedValueOnce(new Response(JSON.stringify({ id: 3, title: 'Rate Limited Resume' }), { status: 200 }));
  ...
  describe('deleteResume with retry', () => {
    it('retries on 504 Gateway Timeout', async () => {
      fetchSpy
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Gateway Timeout' }), { status: 504 }))
    ...
    it('retries on 408 Request Timeout', async () => {
      fetchSpy
        .mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'Request Timeout' }), { status: 408 }))
        .mockResolvedValueOnce(new Response('', { status: 204 }));
